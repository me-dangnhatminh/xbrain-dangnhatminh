...
class TestToolOrchestrator:
    ...
    def test_system_prompt_contains_tool_guidance(self):
        """Test L3 system prompt contains tool selection guidance."""
        mock_tool_executor = Mock(spec=ToolExecutor)
        orchestrator = ToolOrchestrator(mock_tool_executor)
        
        prompt = orchestrator._get_l3_system_prompt()
        
        # Check for key guidance elements
        assert "query_database" in prompt
        assert "get_service_metrics" in prompt
        assert "dữ liệu lịch sử" in prompt.lower() or "historical" in prompt.lower()
        assert "thời gian thực" in prompt.lower() or "real-time" in prompt.lower() or "hiện tại" in prompt.lower()
