...
class TestRAGPipelineRetrieveAndGenerate:
    ...
    def test_retrieve_and_generate_handles_no_chunks(self, rag_pipeline):
        ...
        with patch.object(rag_pipeline.bedrock_agent_runtime, 'retrieve', return_value=empty_response):
            ...
            assert isinstance(response, Response)
            assert 'không tìm thấy' in response.answer.lower() or 'không có' in response.answer.lower()
...
class TestRAGPipelineHelperMethods:
    ...
    def test_format_chunks_as_context(self, rag_pipeline):
        ...
        assert "Thông tin từ cơ sở tri thức" in context
        
        # Check that sources are numbered
        assert "[Nguồn 1: team_platform.md]" in context
        assert "[Nguồn 2: deployment_policy.md]" in context
    ...
    def test_get_l1_system_prompt(self, rag_pipeline):
        """Test that _get_l1_system_prompt() returns proper system prompt."""
        prompt = rag_pipeline._get_l1_system_prompt()
        
        # Check key instructions are present
        assert "GeekBrain" in prompt
        assert "trích dẫn nguồn" in prompt or "citation" in prompt.lower()
