data "archive_file" "api_handler_zip" {
  type        = "zip"
  source_dir  = "${path.module}/../lambdas/api-handler-lambda"
...
data "archive_file" "event_handler_zip" {
  type        = "zip"
  source_dir  = "${path.module}/../lambdas/event-handler-lambda"
  output_path = "${path.module}/../lambdas/event-handler.zip"
}

resource "aws_lambda_function" "event_handler" {
  function_name    = "${var.application}-event-handler"
  role             = aws_iam_role.lambda_role.arn
  handler          = "index.handler"
  runtime          = "python3.10"
  ...
  environment {
    variables = {
      DOCUMENT_TABLE = aws_dynamodb_table.documents.name
      S3_BUCKET      = aws_s3_bucket.app_data.id
      BEDROCK_KB_ID  = aws_bedrockagent_knowledge_base.app_kb.id
      BEDROCK_DS_ID  = aws_bedrockagent_data_source.app_ds.data_source_id
    }
  }
}
